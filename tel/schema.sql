...
--
-- Design notes:
--  * The newline-delimited JSON files under data/raw/ are the append-only
...
CREATE TABLE IF NOT EXISTS deployments (
    deployment_id TEXT PRIMARY KEY,       -- provider:native_id
    provider      TEXT,                   -- github | eas | railway
    project_id    TEXT,
    service       TEXT,                   -- atlas-app, atlas-web, atlas-analytics
    ...
    commit_sha    TEXT,
    branch        TEXT,
    version       TEXT,                   -- runtime version / update group id
    url           TEXT,
    raw_json      TEXT
);
CREATE INDEX IF NOT EXISTS idx_deploy_sha ON deployments(commit_sha);
CREATE INDEX IF NOT EXISTS idx_deploy_env ON deployments(environment);

CREATE TABLE IF NOT EXISTS pull_requests (
    pr_id         TEXT PRIMARY KEY,       -- repo#number
    project_id    TEXT,
    number        INTEGER,
    title         TEXT,
    state         TEXT,
...
-- undone. Detected from commit subjects and GitHub revert PRs.
...
-- Generic outcome rows keyed on session, so analytics, email, social and
-- (later) revenue are rows rather than schema changes.
CREATE TABLE IF NOT EXISTS outcomes (
    outcome_id     INTEGER PRIMARY KEY AUTOINCREMENT,
    dedupe_key     TEXT UNIQUE,
    session_id     TEXT,
    project_id     TEXT,
    source         TEXT,                  -- app_analytics | loops | social | web | stripe
    metric         TEXT,                  -- users_reached, signups, views, ...
    value          REAL,
    unit           TEXT,
    window_start   TEXT,
    window_end     TEXT,
    confidence     TEXT,                  -- reached | live_to | ceiling
    method         TEXT,                  -- how it was derived, in words
    raw_json       TEXT
);
CREATE INDEX IF NOT EXISTS idx_outcome_session ON outcomes(session_id);

-- Maps changed code paths to the analytics surface they affect. Without this
-- the best you can say is "how many users were active after the deploy";
-- with it you can say "how many users touched the thing that changed".
CREATE TABLE IF NOT EXISTS surface_map (
    id          INTEGER PRIMARY KEY AUTOINCREMENT,
    project_id  TEXT,
    path_glob   TEXT,                     -- atlas-app/components/screens/Home.tsx
    surface     TEXT,                     -- Home
    event_name  TEXT,                     -- the analytics event that means "saw it"
    notes       TEXT,
    UNIQUE(project_id, path_glob, event_name)
);
...
-- Model-assigned labels for turns. Kept in its own table so that re-deriving
-- `turns` (which rebuilds from scratch) never discards work that cost money
-- to produce. `prompt_hash` means an edited prompt is reclassified and an
-- unchanged one never is.
CREATE TABLE IF NOT EXISTS turn_classification (
    turn_id       TEXT PRIMARY KEY,
    label         TEXT,                  -- steering | correction | normal
...
-- For each session, a judgement on the skills that were installed and
-- available but never invoked: should they have fired? A skill that was read
-- and then not used is the strongest signal, because the agent considered it
-- and declined - that is a description problem, not an awareness problem.
CREATE TABLE IF NOT EXISTS skill_audit (
    session_id   TEXT NOT NULL,
    skill_name   TEXT NOT NULL,
    verdict      TEXT,                    -- should_have_fired | not_applicable
    confidence   TEXT,                    -- high | medium | low
    reason       TEXT,
    was_read     INTEGER DEFAULT 0,       -- the SKILL.md was opened this session
    model        TEXT,
    audited_at   TEXT,
    PRIMARY KEY (session_id, skill_name)
);
CREATE INDEX IF NOT EXISTS idx_audit_verdict ON skill_audit(verdict);

-- ------------------------------------------------- correction diagnosis ---

-- Why a correction happened, and what would have prevented it. The taxonomy
-- is chosen so every value maps to an action: something to write down,
-- something to change about how work is asked for, or nothing (design
-- iteration has no knowable right answer in advance).
CREATE TABLE IF NOT EXISTS correction_cause (
    turn_id       TEXT PRIMARY KEY,
    session_id    TEXT,
    cause         TEXT,      -- missing_context | stale_context | ambiguous_request
                             -- | wrong_approach | design_iteration | tool_failure
    confidence    TEXT,
    what_was_missing TEXT,   -- the fact the agent did not have, in one line
    suggested_fix TEXT,      -- what to write down or change
    fix_location  TEXT,      -- claude_md | memory | docs | skill | prompt | none
    model         TEXT,
    diagnosed_at  TEXT
);
CREATE INDEX IF NOT EXISTS idx_cause ON correction_cause(cause);

-- ------------------------------------------------------ doc inventory ----

-- Every document in a knowledge base, whether or not anything ever read it.
-- A cold document is only a problem if it was meant to be consulted, so
-- `audience` and `agent_relevance` are what make coldness meaningful:
-- docs/Contacts/*.md going unread by an agent is correct, not a gap.
CREATE TABLE IF NOT EXISTS doc_inventory (
    path            TEXT PRIMARY KEY,
    root            TEXT,
    rel_path        TEXT,
    title           TEXT,
    topic           TEXT,               -- one line, model-written
    audience        TEXT,               -- agent | human | both
    agent_relevance TEXT,               -- high | medium | low
    size_bytes      INTEGER,
    word_count      INTEGER,
    modified_at     TEXT,
    excerpt         TEXT,               -- local, never sent anywhere but the profiler
    model           TEXT,
    profiled_at     TEXT
);
CREATE INDEX IF NOT EXISTS idx_doc_relevance ON doc_inventory(agent_relevance);

-- ---------------------------------------------------------- doc gaps -----

-- Per workflow: documents that would have helped and were never opened while
-- that work was happening. Scoped to the workflow rather than to the vault as
-- a whole, because relevance is only meaningful against a specific piece of
-- work.
CREATE TABLE IF NOT EXISTS doc_gap (
    session_id     TEXT NOT NULL,
    doc_path       TEXT NOT NULL,
    verdict        TEXT,          -- should_have_read | not_relevant
    confidence     TEXT,
    reason         TEXT,
    model          TEXT,
    judged_at      TEXT,
...
CREATE INDEX IF NOT EXISTS idx_docgap_verdict ON doc_gap(verdict);

-- --------------------------------------------------------- narratives ----

-- One line saying what a session was actually trying to do. Every other table
-- here is counts and paths; this is the sentence a human needs to recognise a
-- session without reading its prompts.
...
-- What went wrong in a session and what to change about it. The inputs are
-- mechanical friction signals (rewrite loops, repeated commands, failing
-- tools, cost spikes) computed without a model; this table holds only the
-- interpretation of them. Signals are re-derivable at any time from the
-- normalized tables, so nothing here needs to store them.
CREATE TABLE IF NOT EXISTS session_diagnosis (
    session_id   TEXT NOT NULL,
    seq          INTEGER NOT NULL,      -- rank within the session
    kind         TEXT,                  -- rework_loop | context_loss |
                                        -- missing_automation | brittle_tooling |
                                        -- unclear_target | expensive_detour |
                                        -- working_as_intended
    evidence     TEXT,                  -- the numbers the finding rests on
    finding      TEXT,                  -- what is happening, one sentence
    fix          TEXT,                  -- the change to make
    fix_location TEXT,                  -- claude_md | memory | docs | skill |
                                        -- hook | prompt | tooling | none
...
CREATE INDEX IF NOT EXISTS idx_dx_kind ON session_diagnosis(kind);
CREATE INDEX IF NOT EXISTS idx_dx_sev ON session_diagnosis(severity);

-- Sessions the model looked at and found nothing wrong with. Without this,
-- every re-run pays again for the sessions that are genuinely fine.
CREATE TABLE IF NOT EXISTS session_dx_clean (
...
-- Questions asked about a session and the answers given. Kept so a line of
-- enquiry survives closing the TUI, and appended to a sidecar so it survives
-- a database rebuild like everything else here that costs money to produce.
CREATE TABLE IF NOT EXISTS session_chat (
    session_id TEXT NOT NULL,
    seq        INTEGER NOT NULL,
    role       TEXT,                     -- user | assistant
    text       TEXT,
    model      TEXT,                     -- null on the user's own messages
    ts         TEXT,
    PRIMARY KEY (session_id, seq)
);
