...
running a real session into this project's collector and dumping every
...
> **[verified]** Because the last two default to *false*, `app.version` and
...
|---|---|---|
| `grpc` | `http://localhost:4317` | default choice here; lowest overhead |
| `http/protobuf` | `http://localhost:4318/v1/{logs,metrics,traces}` | sends `Content-Length` |
| `http/json` | same | required by this project's `--no-docker` fallback receiver |
...
Irrelevant for a loopback collector.
...
>
> This project works around it two ways — see the README's *Project mapping*
...
Extended thinking is *always* omitted from API body logs and is not unlocked by
...
tool *names*, counts and durations, and nothing else.

`OTEL_LOG_TOOL_DETAILS=1` is therefore **on** in the generated
`.env.telemetry`. The consequence, stated plainly:
...
- So with tool details on, **short excerpts of file content can reach
  `data/raw/*.jsonl`**. They never reach the normalized database: the ingester
  applies a metadata allowlist first. See `PRIVACY.md`.
- `./setup.sh --minimal` turns it off if that trade is unacceptable.
...
| files *created* vs modified | **no** | not exposed at all; must be inferred locally (README) |
...
A real *interactive* session (as opposed to `claude -p`) emitted four event
...
**[verified]**, and all handled by `tel/ingest.py`:
